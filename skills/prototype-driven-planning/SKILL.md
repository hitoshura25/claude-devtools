...
2. **Identify the core technical risk.** What's the one thing that, if it doesn't work,
   kills the feature? This becomes the prototype's target.

3. **Research.** Using whatever tools are available (web search, documentation, MCP
   servers, or training knowledge), investigate best practices for the technologies
   involved. Focus on the specific technical risk, not general knowledge.

4. **Propose the prototype scope.** Present to the user:
   - What the prototype will prove
   - What it will NOT include (no edge cases, no polish)
   - Which files it will create and where (`prototypes/<feature-name>/`)
   - The core technical risk it addresses

**STOP.** Present your discovery findings and prototype proposal. Wait for user
